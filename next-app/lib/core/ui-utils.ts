// UI utility functions for loading states, errors, and messages
type NotificationType = 'success' | 'error' | 'warning' | 'info';

interface FormData {
  [key: string]: any;
}

export class UIUtils {
  private loadingSpinner: HTMLElement | null;
  private errorMessage: HTMLElement | null;
  private bulletsSection: HTMLElement | null;

  constructor() {
    if (typeof window !== 'undefined') {
      this.loadingSpinner = document.getElementById('loadingSpinner');
      this.errorMessage = document.getElementById('errorMessage');
      this.bulletsSection = document.getElementById('bulletsSection');
    } else {
      this.loadingSpinner = null;
      this.errorMessage = null;
      this.bulletsSection = null;
    }
  }

  // Show/hide loading spinner
  showLoading(show: boolean): void {
    if (this.loadingSpinner) {
      this.loadingSpinner.classList.toggle('active', show);
    }
  }

  // Show error message
  showError(message: string): void {
    if (this.errorMessage) {
      this.errorMessage.textContent = message;
      this.errorMessage.classList.add('active');
    }
  }

  // Hide error message
  hideError(): void {
    if (this.errorMessage) {
      this.errorMessage.classList.remove('active');
    }
  }

  // Hide bullets section
  hideBullets(): void {
    if (this.bulletsSection) {
      this.bulletsSection.classList.remove('active');
    }
  }

  // Show success message near the submit button
  showSuccessMessage(message: string, redirectUrl: string | null = null, redirectDelay: number = 3000): void {
    if (typeof window === 'undefined') return;
    
    // Create or reuse success message element in submit section
    let successDiv = document.getElementById('submitSuccessMessage') as HTMLDivElement | null;
    if (!successDiv) {
      successDiv = document.createElement('div');
      successDiv.id = 'submitSuccessMessage';
      successDiv.className = 'submit-success-message';
      
      // Insert in the submit container, after the button
      const submitContainer = document.querySelector('.submit-container') as HTMLElement | null;
      const submitButton = submitContainer?.querySelector('button') as HTMLButtonElement | null;
      if (submitContainer && submitButton) {
        submitContainer.insertBefore(successDiv, submitButton.nextSibling);
      }
    }
    
    successDiv.textContent = message;
    successDiv.classList.add('active');
    
    // Auto-hide and optionally redirect
    setTimeout(() => {
      successDiv.classList.remove('active');
      
      if (redirectUrl) {
        window.location.href = redirectUrl;
      }
    }, redirectDelay);
  }

  // Show temporary notification
  showNotification(message: string, type: NotificationType = 'info', duration: number = 5000): HTMLDivElement | null {
    if (typeof window === 'undefined') return null;
    
    const notification = document.createElement('div');
    notification.className = `notification notification-${type}`;
    notification.textContent = message;
    
    // Add to page
    document.body.appendChild(notification);
    
    // Style the notification
    Object.assign(notification.style, {
      position: 'fixed',
      top: '20px',
      right: '20px',
      padding: '12px 16px',
      borderRadius: '4px',
      color: 'white',
      fontSize: '14px',
      fontWeight: '500',
      zIndex: '1000',
      maxWidth: '300px',
      wordWrap: 'break-word'
    });

    // Set background color based on type
    const colors: Record<NotificationType, string> = {
      success: '#10B981',
      error: '#EF4444',
      warning: '#F59E0B',
      info: '#3B82F6'
    };
    notification.style.backgroundColor = colors[type] || colors.info;
    
    // Auto-remove
    setTimeout(() => {
      if (notification.parentNode) {
        notification.parentNode.removeChild(notification);
      }
    }, duration);
    
    return notification;
  }

  // Validate that form has content before processing
  validateFormContent(formData: FormData): boolean {
    const hasContent = Object.values(formData).some(value => 
      value && value.toString().trim().length > 0
    );
    
    if (!hasContent) {
      this.showError('Please fill in at least one field before processing');
      return false;
    }
    
    return true;
  }

  // Disable/enable form during processing
  setFormDisabled(disabled: boolean): void {
    if (typeof window === 'undefined') return;
    
    const form = document.getElementById('updateForm') as HTMLFormElement | null;
    if (form) {
      const formElements = form.querySelectorAll('input, textarea, button, select');
      formElements.forEach(element => {
        (element as HTMLInputElement | HTMLTextAreaElement | HTMLButtonElement | HTMLSelectElement).disabled = disabled;
      });
    }
  }

  // Show confirmation dialog
  confirm(message: string, title: string = 'Confirm'): boolean {
    if (typeof window === 'undefined') return false;
    return window.confirm(`${title}\n\n${message}`);
  }

  // Get element safely (returns null if not found)
  getElement(id: string): HTMLElement | null {
    if (typeof window === 'undefined') return null;
    return document.getElementById(id);
  }

  // Check if element exists and is visible
  isElementVisible(id: string): boolean {
    if (typeof window === 'undefined') return false;
    const element = document.getElementById(id);
    return element !== null && element.offsetParent !== null;
  }

  // Dashboard-specific UI state management
  showDashboardLoading(show: boolean): void {
    if (typeof window === 'undefined') return;
    const loadingEl = document.getElementById('dashboardLoading');
    if (loadingEl) {
      loadingEl.style.display = show ? 'block' : 'none';
    }
  }

  showDashboardError(message?: string): void {
    if (typeof window === 'undefined') return;
    const errorEl = document.getElementById('dashboardError');
    if (errorEl) {
      if (message) {
        const messageEl = errorEl.querySelector('p');
        if (messageEl) {
          messageEl.textContent = message;
        }
      }
      errorEl.style.display = 'block';
    }
  }

  hideDashboardError(): void {
    if (typeof window === 'undefined') return;
    const errorEl = document.getElementById('dashboardError');
    if (errorEl) {
      errorEl.style.display = 'none';
    }
  }

  showDashboardEmpty(show: boolean): void {
    if (typeof window === 'undefined') return;
    const emptyEl = document.getElementById('dashboardEmpty');
    if (emptyEl) {
      emptyEl.style.display = show ? 'block' : 'none';
    }
  }

  hideDashboardEmpty(): void {
    this.showDashboardEmpty(false);
  }

  showDashboardTabs(): void {
    if (typeof window === 'undefined') return;
    const tabsContainer = document.getElementById('dashboardTabs');
    if (tabsContainer) {
      tabsContainer.style.display = 'flex';
    }
  }

  hideDashboardTabs(): void {
    if (typeof window === 'undefined') return;
    const tabsContainer = document.getElementById('dashboardTabs');
    if (tabsContainer) {
      tabsContainer.style.display = 'none';
    }
  }
}

// Create singleton instance
let uiUtilsInstance: UIUtils | null = null;

export function getUIUtils(): UIUtils {
  if (!uiUtilsInstance) {
    uiUtilsInstance = new UIUtils();
  }
  return uiUtilsInstance;
}

// For backward compatibility with window.UIUtils and window.uiUtils
if (typeof window !== 'undefined') {
  (window as any).UIUtils = UIUtils;
  (window as any).uiUtils = getUIUtils();
}